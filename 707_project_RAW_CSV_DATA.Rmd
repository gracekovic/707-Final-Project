...
BRFSS2015_csv = BRFSS2015_csv %>% dplyr::select(SEX, X_AGEG5YR, MENTHLTH, INCOME2, PHYSHLTH, HLTHPLN1)

# Structure of dataset and summary statistics of all variables 
str(BRFSS2015_csv)
summary(BRFSS2015_csv)

# 3301 missing values for income, 1 missing value for physical health
colSums(is.na(BRFSS2015_csv)) 

# Make incoome level a factor
BRFSS2015_csv$INCOME2 = as.factor(BRFSS2015_csv$INCOME2) 

# # Rename Age variable 
# BRFSS2015_csv$AGEG5YR = BRFSS2015_csv$X_AGEG5YR
```
...
```{r}
table(BRFSS2015_csv$SEX)
# 1 = Male 
# 2 = Female 

# BRFSS2015_csv[, "newSEX"] = BRFSS2015_csv[, "SEX"]-1

# Rename 1 and 2 as "Male" and "Female"
BRFSS2015_csv$SEX_cat = factor(BRFSS2015_csv$SEX, 
                                 levels = c(1, 2), 
...
BRFSS2015_csv$AGE_14cat = factor(BRFSS2015_csv$X_AGEG5YR, 
...
sum(BRFSS2015_csv$AGE_14cat == "Don't know/Refused/Missing")
...
```{r}
summary(BRFSS2015_csv$INCOME2)

# Rename each level of income using income ranges in codebook
BRFSS2015_csv$INCOME_10cat = factor(BRFSS2015_csv$INCOME2, 
...
table(BRFSS2015_csv$INCOME_10cat)
```

```{r}
# Distribution of income level, EXCLUDING values of 77 ("Don't know/Not sure") and 99 ("Refused")
BRFSS2015_csv %>% dplyr::filter(INCOME_10cat != "Don't_know/Not_sure" & INCOME_10cat != "Refused") %>% 
...
table(BRFSS2015_csv$MENTHLTH) # There are 301,076 individuals who reported "None" (zero days in the past month of feeling mentally not good)
...
```{r}
# Box plot of Mental Health Across Income Level - "Refused" group for income EXCLUDED
BRFSS2015_csv %>% 
  filter(MENTHLTH != 88 & 
           MENTHLTH != 99 & 
           MENTHLTH != 77 &
...
summary(BRFSS2015_csv$PHYSHLTH)

table(BRFSS2015_csv$PHYSHLTH) # There are 274,143 ppl who reported  "None" (zero days of feeling mentally unwell)
...
BRFSS2015_csv %>% filter(is.na(PHYSHLTH))
  
```

```{r}
# Distribution of Physical Health Status
BRFSS2015_csv %>% 
  filter(!is.na(PHYSHLTH)) %>% # Excluded the 1 missing value for physical health
  #filter(PHYSHLTH != "88") %>% # Excluded individuals with "None"
...
```{r}
summary(BRFSS2015_csv$HLTHPLN1)
table(BRFSS2015_csv$HLTHPLN1)
...
```{r}
# Rename 0 and 1 as "Male" and "Female"
BRFSS2015_csv$HLTHPLN1_cat = factor(BRFSS2015_csv$HLTHPLN1, 
                                 levels = c(1, 2), 
                                 labels = c("Yes", "No"))
...
BRFSS2015_csv %>% 
  ...
  labs(subtitle = "Do you have any kind of health care coverage, including health insurance, prepaid plans such as HMOs, or government plans such as Medicare, or Indian Health Services")
```
...
```{r}
# Removes values of 77 ("Don't know/Not sure"), 99 ("Refused") in Income variable AND all NA values (3300)
BRFSS2015_csv_no_77_99_income = BRFSS2015_csv[BRFSS2015_csv$INCOME2 != 77 & BRFSS2015_csv$INCOME2 != 99, ]
```
...
```{r}
BRFSS2015_csv_NA_removed = BRFSS2015_csv %>% na.omit()
```
